...
//
// Version: $Id: EUTelMille.cc 2898 2013-08-07 09:58:36Z hperrey $
/*
...
//      {
        ...
        for (int nHits = 0; nHits < int(TrackHitsHere.size()); nHits++) {
          ...
          {
             //Stefano =========================================
	     int iden = guessSensorID(HitHere);
	     nPlaneHere = iden;
	     if(nPlaneHere > 19) nPlaneHere -= 17;
	     else if(nPlaneHere > 2) nPlaneHere += 4;
	     //std::cout <<  __LINE__ << "] SensorID: " << iden << "\t nPlaneHere: " << nPlaneHere << "\n";
            // fill hits to arrays
    ...
    for (int track = 0; track < _nTracks; track++) {
      ...
      if(_alignMode == 3)
	    ...
	    if( abs(x)>1e-06 && abs(y)>1e-06 && abs(z)>1e-06 && index_firsthit == 0 )
	      ...
	      //if ( xresid < -500 || xresid > 500)
              {
                std::cout << __LINE__ << "] xres= " << xresid << "\n";
                continue;
              }
            if ( yresid < _residualsYMin[help] || yresid > _residualsYMax[help]) 
	      //if ( yresid < -500 || yresid > 500) 
              {
	        std::cout << __LINE__ << "] yres= " << yresid << "\n";
                continue;
              }
 
            if(!excluded)
              {
	        //std::cout << __LINE__ << "] help: " << help << "\n";
		//std::cout << __LINE__ << "] resX: " << _resolutionX[help] << "\n";
		//std::cout << __LINE__ << "] resY: " << _resolutionX[help] << "\n";
		//std::cout << __LINE__ << "] resZ: " << _resolutionX[help] << "\n";
		...
		{
		  //std::cout << __LINE__ << "] absx= " << abs(x) << "\n";
		  //std::cout << __LINE__ << "] absy= " << abs(y) << "\n";
		  //std::cout << __LINE__ << "] absz= " << abs(z) << "\n";
		}
		...
		//Stefano
	     ...
	     std::cout << __LINE__ << "] _nPlanes= " << _nPlanes << "\n";
	     std::cout << __LINE__ << "] = mean_n=" << mean_n << "\n";
	     std::cout << __LINE__ << "] = getAllowedMissingHits()=" << getAllowedMissingHits() << "\n";
             continue;
          }
	  //std::cout << __LINE__ << "Stefano diff mean" << "\n";
...
int EUTelMille::guessSensorID( TrackerHit *hit ) {
...
void EUTelMille::end() {
  ...
  if (_generatePedeSteerfile) {
    ...
    for(unsigned int iDetector = 0; iDetector < _nPlanes; iDetector++ ) {
      ...
      int sensorID = _orderedSensorID.at( iDetector );
      //Stefano
      std::cout << __LINE__ << "] iDetector: " << iDetector << "\n";
      std::cout << __LINE__ << "] sensorID: " << sensorID << "\n";
      if(iDetector > 2 && iDetector < 7) continue;
    ...
    if (steerFile.is_open()) {
      ...
      //Stefano
      std::cout << __LINE__ << "] firstnotexcl: " << firstnotexcl << "\n";
      std::cout << __LINE__ << "] lastnotexcl: "  << lastnotexcl << "\n";
      // calculate average
      ...
      for (unsigned int help = 0; help < _nPlanes; help++) {
        ...
        bool excluded = 0; // flag for excluded planes
        ...
        if (excluded == 0) {
          ...
          bool fixed = false;
          //Stefano
	  //if(counter == 0 || counter == 5) fixed = true;
          ...
          if( fixed || (_FixedPlanes.empty() && (help == firstnotexcl || help == lastnotexcl) ) )
            ...
            if (_alignMode == 1) {
              ...
              if (_usePedeUserStartValues == 0)
                {
      		  std::cout << __LINE__ << "] help: " << help << "\n";
		  std::cout << __LINE__ << "] counter: " << counter << "\n";
...
//	      int iden = counter;
	      //if(iden > 2) iden += 4;
//	      constant->setSensorID( iden );
              ++ counter;
              constantsCollection->push_back( constant );
	      streamlog_out ( MESSAGE0 ) << "pippo" << endl;
