...
// Version $Id: EUTelHitMaker.cc 2878 2013-08-02 06:55:11Z hperrey $
/*
...
_siOffsetYMap()
{
  ...
  //Stefano stefano
  registerOptionalParameter ("GapX","Gap X", _GapX, std::vector<float>(static_cast<float> (3), -1) );
  registerOptionalParameter ("GapY","Gap Y", _GapY, std::vector<float>(static_cast<float> (3), -1) );
...
//    _siOffsetYMap.push_back( 0.) ;
  ...
  if( _GapX.size() % 3 != 0) 
  {
     streamlog_out (ERROR) << "Wrong number of input values in GapX; check your steering file !" <<endl;
  }
  else
  {
    streamlog_out (MESSAGE) << "GapX initialised with " << _GapX.size() << " elements" << endl;
   
    for(unsigned int i = 0; i < _GapX.size(); i=i+3)
    {
       int ivector = i % 3;
       int iSensor = _GapX.at(i);
       printf("%5d : ivector %5d sensorID: %2d \n", i, ivector, iSensor );
       _GapXMap[ iSensor ].push_back(_GapX.at(i+1));
       _GapXMap[ iSensor ].push_back(_GapX.at(i+2));
       std::cout <<  i << " : _GapXMap[" << iSensor <<"] " << _GapXMap[ iSensor ][i/3*2] << " " << _GapXMap[ iSensor ][i/3*2+1] << std::endl;
    }
  }
  
  if( _GapY.size() % 3 != 0) 
  {
     streamlog_out (ERROR) << "Wrong number of input values in GapY; check your steering file !" <<endl;
  }
  else
  {
    streamlog_out (MESSAGE) << "GapY initialised with " << _GapY.size() << " elements" << endl;
   
    for(unsigned int i = 0; i < _GapY.size(); i=i+3)
    {
       int ivector = i % 3;
       int iSensor = _GapY.at(i);
       printf("%3d : ivector %3d sensorID: %2d \n", i, ivector, iSensor );
       _GapYMap[ iSensor ].push_back(_GapY.at(i+1));
       _GapYMap[ iSensor ].push_back(_GapY.at(i+2));
       std::cout <<  i << " : _GapYMap[" << iSensor <<"] " << _GapYMap[ iSensor ][i/3*2] << " " << _GapYMap[ iSensor ][i/3*2+1] << std::endl;
    }
  }
...
//          if( yCoG > apixPixel.getYCoord() ) yCoG = apixPixel.getYCoord() ;
  ...
  */
      ...
      //Stefano, stefano changes for MPP Goe quad modules spacing between chips
  
      float xOffset=0.,yOffset=0.;
/*
      if(type == kEUTelAPIXClusterImpl)
      {
        if(xCluSeed==79 )  xOffset=0.1;
        if(xCluSeed==80 )  xOffset=0.3;
        if(xCluSeed>80  )  xOffset=0.4 ;
        if(yCluSeed>335 )  yOffset=1.6; 
      }
*/
      std::map< int,FloatVec >::iterator gapX = _GapXMap.find(detectorID);
      if(gapX != _GapXMap.end() )
      {
        //std::cout << "appling gapX for pixel(col:row) " << xCluSeed<< ":" <<yCluSeed << std::endl;
        for(unsigned int i=0; i < gapX->second.size(); i+=2)
        {
          if(xCluSeed > gapX->second[i] ) xOffset+=gapX->second[i+1];
	  
	  //  std::cout << "offset=" << xOffset << " because " << xCluSeed << " > " << gapX->second[i] << std::endl;
	  
	  //std::cout << "offset=" << xOffset << std::endl;
        } 
      }
      std::map< int,FloatVec >::iterator gapY = _GapYMap.find(detectorID);
      if( gapY != _GapYMap.end() )
      {
        //std::cout << "appling gapY for pixel(col:row) " << xCluSeed<< ":" <<yCluSeed << std::endl;
        for(unsigned int i=0; i < gapY->second.size(); i+=2)
        {
          if(yCluSeed>gapY->second[i] )  yOffset+=gapY->second[i+1];
	  //std::cout << "offset=" << yOffset << std::endl;
        } 
      } 
 
      xDet = (xCoG + 0.5) * xPitch + xOffset;
      yDet = (yCoG + 0.5) * yPitch + yOffset;
