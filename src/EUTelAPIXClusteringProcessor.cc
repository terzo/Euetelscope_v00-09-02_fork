...
std::string EUTelAPIXClusteringProcessor::_clusterHolesXName		= "Xclusterholes";
std::string EUTelAPIXClusteringProcessor::_clusterHolesYName		= "Yclusterholes";
...
/****************************************************************************
** Clustering algorithm rewritten by Stefano Terzo (stefano.terzo@mpp.mpg.de)
** 
**
...
//streamlog_out ( MESSAGE1 ) << "PixelInfo:  " << apixPixel.getXCoord() << " " << apixPixel.getYCoord() << " " << apixPixel.getSignal() << " " << apixPixel.getChip() << " " << apixPixel.getTime()<< endl;
			...
			int clusterID = 0;												    
			//std::cout << __LINE__ << "] " << "start clustering" << "\n";									    
  			while( apixPixelVec.size() != 0 )										    
  			{	
			  //std::cout << __LINE__ << "] cluster number: " << clusterID << "\n";					
			  ...
			  //std::vector<EUTelAPIXSparsePixel*> apixClusterVec;						     			    
  			  //apixClusterVec.push_back( (*apixPixelVec.begin()) );
			  apixCluster->addSparsePixel( (*apixPixelVec.begin()) );							    
  			  apixPixelVec.erase( apixPixelVec.begin() );							    
  			  //for(unsigned int clu =0; clu < apixClusterVec.size(); clu++ )
			  for(unsigned int clu =0;clu < apixCluster->size(); ++clu)
  			  {														    
  			    /*
			    int cRow = apixClusterVec[clu]->getYCoord();										
  			    int cCol = apixClusterVec[clu]->getXCoord() ;									      
			    int cL1id= apixClusterVec[clu]->getTime();
			    */
			    apixCluster->getSparsePixelAt(clu, &apixPixel);
			    short cRow = apixPixel.getYCoord();
			    short cCol = apixPixel.getXCoord() ;
			    short cL1id= apixPixel.getTime();
			    //std::cout << __LINE__ << "] bum cluster " << "\n";										    
  			    for(unsigned int pix=0; pix < apixPixelVec.size(); pix++)							
  			    {				
			      int pRow = apixPixelVec[pix]->getYCoord(); 
                              int pCol = apixPixelVec[pix]->getXCoord() ;
			      int pL1id= apixPixelVec[pix]->getTime();
			      int xDist = abs(cCol - pCol);
			      int yDist = abs(cRow - pRow);
  			      if ( ( xDist <= _minXDistance )  && ( yDist <= _minYDistance ) &&     
				   ( abs(cL1id- pL1id)<= _minLVL1 ) )      
  			      { 				
			        if( _minDiagDistance > -1 && 
				    xDist != 0 && yDist != 0 && 
				    max(xDist,yDist) > _minDiagDistance)  continue;
				
  				//apixClusterVec.push_back( apixPixelVec[pix] );
				apixCluster->addSparsePixel( apixPixelVec[pix] );
  				apixPixelVec.erase( (apixPixelVec.begin()+pix) );
				pix--;
  			      }												    
  			    }														    
  			  }
			  		
			  if ( addCluster(apixCluster, sensorID) ) {
			      float x = 0;
    			      float y = 0;
    			      int xsize = 0;
    			      int ysize = 0;
			      ...
			      //std::cout << __LINE__ << "] added " << "\n";
    			      ...
    			      apixCluster->getRealClusterSize(xRsize,yRsize);
    			      ...
    			      if (x > 0 && x < 10000 && y > 0 && y < 10000)
    			      {
			      	 ...
			      	 _totClusterMap[ sensorID ] += 1;
      			      	 ...
      			      	 //zsDataEncoder["xHoles"]	  = xsize - xRsize;
      			      	 //zsDataEncoder["yHoles"]	  = ysize - yRsize;
...
bool EUTelAPIXClusteringProcessor::addCluster (eutelescope::EUTelSparseClusterImpl< eutelescope::EUTelAPIXSparsePixel >  *apixCluster, int& sensorID)
{
  int xsize0,ysize0;
  ...
  if( bSensorID && _ClusterLimitsMap.find(sensorID) != _ClusterLimitsMap.end() )
  {
    //std::cout << "---------------------------------------------------------------------------> " << _ClusterLimitsMap[sensorID].size() << "\n";
    if(
	  ...
	  try {
		...
		for ( int iCluster = _initialClusterCollectionSize; iCluster < clusterCollectionVec->getNumberOfElements(); iCluster++ ) {
			...
			if (static_cast< int >(type) == static_cast< int >(kEUTelAPIXClusterImpl)  ) {
				...
				tempHistoName = _clusterHolesXName + "_d" + to_string( sensorID );
				(dynamic_cast<AIDA::IHistogram2D*> (_aidaHistoMap[tempHistoName]))->fill(xSize,xSize-xRsize);
				
				tempHistoName = _clusterHolesYName + "_d" + to_string( sensorID );
				(dynamic_cast<AIDA::IHistogram2D*> (_aidaHistoMap[tempHistoName]))->fill(ySize,ySize-yRsize);
...
void EUTelAPIXClusteringProcessor::bookHistos() {
	...
	for (size_t iDetector = 0; iDetector < _sensorIDVec.size(); iDetector++) {
		...
		string clusterHolesXTitle = "Clusterholes in X";
		tempHistoName = _clusterHolesXName + "_d" + to_string( sensorID );
		AIDA::IHistogram2D * clusterXHoles = AIDAProcessor::histogramFactory(this)->createHistogram2D( (basePath + tempHistoName).c_str(), xBin, xMin, xMax,xBin, xMin, xMax);
		_aidaHistoMap.insert(make_pair(tempHistoName, clusterXHoles));
		clusterXHoles->setTitle(clusterHolesXTitle.c_str());

		string clusterHolesYTitle = "Clusterholes in Y";
		tempHistoName = _clusterHolesYName + "_d" + to_string( sensorID );
		AIDA::IHistogram2D * clusterYHoles = AIDAProcessor::histogramFactory(this)->createHistogram2D( (basePath + tempHistoName).c_str(), yBin, yMin, yMax,yBin, yMin, yMax);
		_aidaHistoMap.insert(make_pair(tempHistoName, clusterYHoles));
		clusterYHoles->setTitle(clusterHolesYTitle.c_str());
